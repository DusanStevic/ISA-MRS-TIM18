...
public class RACController {
	...
	public ResponseEntity<RentACar> editHotelImage(@RequestBody ImageDTO image) {
		RentacarAdmin user = (RentacarAdmin) this.userDetailsService.loadUserByUsername(SecurityContextHolder.getContext().getAuthentication().getName());
	...
	public ResponseEntity<RentACar> saveChangesRentACar(@RequestBody RentACarDTO rentacar) {
		...
		r.setDescription(rentacar.getRentacarPromotionalDescription());
