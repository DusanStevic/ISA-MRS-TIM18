	...
	@Query("SELECT DISTINCT a FROM Airline a " +
			   ...
			   "AND a.id in " +
			   		...
			   		"AND f.id in "+
			   			...
			   			"HAVING COUNT(s.id) >= ?5)) ")
		ArrayList<Airline> findByNameAndAddressAndDateAndBeds(String name, String start, String end, Date checkin, int beds);
	...
	@Query("SELECT DISTINCT a FROM Airline a " +
			   "WHERE a.id in " +
			   		...
			   		"AND f.id in "+
			   			...
			   			"GROUP BY s.flight.id "+
			   			"HAVING COUNT(s.id) >= ?4)) ")
		ArrayList<Airline> findByAddressAndDateAndBeds(String start, String end, Date checkin, int beds);
}
